class Rack::Attack
  ...
  if ActiveModel::Type::Boolean.new.cast(ENV.fetch('ENABLE_RACK_ATTACK_WIDGET_API', true))
    ...
    # Лимит поднят с апстримовых 6 — у виджета высокий легитимный трафик.
    # Имя метода взято из апстрима: там исправлена опечатка path_without_extentions.
    throttle('api/v1/widget/conversations', limit: 50, period: 12.hours) do |req|
    ...
    # Лимит поднят с апстримовых 5 — иначе виджет отсекается на легитимном трафике.
    throttle('widget?website_token={website_token}&cw_conversation={x-auth-token}', limit: 5000, period: 1.hour) do |req|
      req.ip if req.path_without_extensions == '/widget' && ActionDispatch::Request.new(req.env).params['cw_conversation'].blank?
    end

    ## Prevent Transcript Bombing on Widget API ### (новое в 4.16.2, оставляем как есть)
